...
const workMutationOutputSchema = `{
  ...
  "type":"object","additionalProperties":false,"required":["schemaVersion","status"],
  "properties":{"schemaVersion":{"const":"1"},"status":{"enum":["applied","unchanged","rejected","error"]},"operation":{"type":"object","additionalProperties":false,"required":["id","replayed","changed","committedAt","clientAttribution"],"properties":{"id":{"type":"string","format":"uuid"},"replayed":{"type":"boolean"},"changed":{"type":"boolean"},"committedAt":{"type":"string","format":"date-time"},"clientAttribution":{"type":"object","additionalProperties":false,"required":["harness","model","source"],"properties":{"harness":{"type":"string","minLength":1,"maxLength":128},"harnessVersion":{"type":"string","minLength":1,"maxLength":64},"model":{"type":"string","minLength":1,"maxLength":128},"source":{"const":"client-reported"}}}}},"createdReferences":{"type":"object","additionalProperties":{"$ref":"#/$defs/issueRef"}},"workItem":{"$ref":"#/$defs/workItemResult"},"workPlan":{"$ref":"#/$defs/workPlanResult"},"selectedContext":{"$ref":"#/$defs/planContextResult"},"currentResultStatus":{"enum":["available","unavailable","projection_unavailable"]},"problem":{"$ref":"#/$defs/problem"}},
  "allOf":[
