...
func TestInspectItemUnplannedAndMultiPlanState(t *testing.T) {
	...
	assert.Equal(t, "disabled", inspection.WorkItem.ProjectMemberships[0].State)
	assert.Equal(t, "draft", inspection.WorkItem.ProjectMemberships[1].State)
	assert.Equal(t, "active", inspection.WorkItem.ProjectMemberships[2].State)
